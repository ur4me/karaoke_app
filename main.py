...
# # #         /* 브랜드 헤더 */
# # #         .brand-header { 
# # #             padding: 24px 20px; border-bottom: 1px solid var(--border-color); background-color: #0c0c10;
...
# # #         /* 패널이 닫혔을 때 나타나는 열기 버튼 (왼쪽 패널 위에 고정) */
# # #         .open-panel-btn {
# # #             display: none; /* 기본 숨김 */
# # #             position: fixed;
# # #             top: 20px;
...
# # #                 </div>
# # #                 <!-- 오른쪽 패널 닫기 버튼 -->
# # #                 <button id="closePanelBtn" class="toggle-panel-btn">메뉴 닫기</button>
# # #             </div>
...
# # #     <!-- 패널이 닫혔을 때 나타나는 메뉴 열기 버튼 (고정 위치) -->
# # #     <button id="openPanelBtn" class="open-panel-btn">메뉴 열기</button>
...
# # #             openPanelBtn.style.display = 'block';  // 고정 버튼 보이기
# # #         });

# # #         openPanelBtn.addEventListener('click', () => {
# # #             rightPanel.classList.remove('hidden');
# # #             openPanelBtn.style.display = 'none';   // 고정 버튼 숨기기
# # #         });
# # #         // ===================================
# # #     </script>
# # # </body>
...
# #             color: #000;
# #             box-shadow: 0 0 15px rgba(166, 124, 255, 0.6);
# #         }

# #         /* 패널이 닫혔을 때 나타나는 열기 버튼 (고정) */
# #         .open-panel-btn {
# #             display: none; /* 기본 숨김 */
# #             position: fixed;
# #             top: 20px;
...
# #                         </svg>
# #                     </div>
# #                     <div class="neon-title">OLPARK KARAOKE</div>
# #                 </div>
# #             </div>
...
# #                     <button id="fullscreenBtn" class="control-btn fullscreen-btn">⛶ 전체화면</button>
# #                 </div>
# #                 <button id="closePanelBtn" class="control-btn close-panel-btn">메뉴 닫기</button>
# #             </div>
# #         </div>
# #     </div>
...
# #     socketserver.ThreadingTCPServer.allow_reuse_address = True
# #     with socketserver.ThreadingTCPServer(("0.0.0.0", port), RequestHandler) as httpd:
# #         print(f"\n🎤 OLPARK KARAOKE 서버가 시작되었습니다! (Render 배포 대응 버전)")
# #         print(f"👉 접속 주소: http://localhost:{port}\n")
# #         if not is_render:
# #             threading.Timer(1.0, lambda: webbrowser.open(f"http://localhost:{port}")).start()
# #         httpd.serve_forever()
...
#         /* 패널이 닫혔을 때 나타나는 열기 버튼 (하단 오른쪽 고정) */
#         .open-panel-btn {
#             display: none; /* 기본 숨김 */
#             position: fixed;
#             bottom: 20px;
...
#     <!-- 패널이 닫혔을 때 나타나는 메뉴 열기 버튼 (하단 오른쪽 고정) -->
#     <button id="openPanelBtn" class="open-panel-btn">메뉴 열기</button>
...
# 쥬피터 노트북 환경 호환성 수정 (__file__이 없을 경우 현재 작업 폴더 기준)
try:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
except NameError:
    BASE_DIR = os.getcwd()
...
<body>
    ...
    <div class="app-container">
        ...
        <div class="left-panel">
            <div id="placeholder" class="player-placeholder">
                <div class="mic-icon">🎤</div>
                <h2 id="placeholderTitle" style="color: white; font-weight: 800; font-size: 1.5rem; margin-bottom:10px;">리모컨으로 곡을 예약해주세요</h2>
                <p id="placeholderSub" style="color: var(--neon-cyan);">우측 검색창에 가수나 제목을 입력하세요</p>
            </div>
    ...
    <script>
        ...
        function onPlayerError(event) {
            if (event.data === 101 || event.data === 150) {
                ...
                if (!newWindow || newWindow.closed || typeof newWindow.closed === 'undefined') {
                    alert("🔒 브라우저의 팝업이 차단되어 유튜브 창을 열지 못했습니다.\\n주소창 우측 상단의 '팝업 허용'을 체크해 주세요!");
                }
                
                // 💡 새 창에서 노래 부르는 동안 본체 앱은 다음 곡으로 넘기지 않고 대기시킵니다.
                isPlayerIdle = true;
                document.getElementById('placeholderTitle').innerText = "외부 유튜브 창에서 곡이 재생 중입니다";
                document.getElementById('placeholderSub').innerText = "노래를 마친 후 하단의 [⏭ 다음 곡] 버튼을 눌러주세요.";
                document.getElementById('placeholder').style.display = 'block';
                player.stopVideo(); 
                
            } else {
                // 다른 일반 에러의 경우에만 즉시 다음 곡 재생
                alert("영상을 재생할 수 없어 다음 곡으로 넘어갑니다.");
                playNextSong();
            }
        }
        ...
        function playNextSong() {
            // 안내 멘트 원상복구
            document.getElementById('placeholderTitle').innerText = "리모컨으로 곡을 예약해주세요";
            document.getElementById('placeholderSub').innerText = "우측 검색창에 가수나 제목을 입력하세요";
        ...
        // 💡 [수정] 대기(Idle) 상태에서도 작동할 수 있도록 제약 조건 수정
        document.getElementById('skipBtn').addEventListener('click', () => {
            if (!isShowingScore) {
                playNextSong(); 
            }
...
def run_server():
    ...
    socketserver.ThreadingTCPServer.allow_reuse_address = True
    with socketserver.ThreadingTCPServer(("0.0.0.0", port), RequestHandler) as httpd:
        print(f"\n🎤 OLPARK KARAOKE 서버가 시작되었습니다!")
        print(f"👉 접속 주소: http://localhost:{port}\n")
        print(f"🛑 서버를 종료하려면 쥬피터 상단의 '정지(Stop/Interrupt)' 버튼을 누르세요.")
        if not is_render:
            threading.Timer(1.0, lambda: webbrowser.open(f"http://localhost:{port}")).start()
